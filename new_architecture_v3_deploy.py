...
def extract_latest_json(text: str):
    """
    Extract the last valid JSON object from a string.
    Returns a Python dict, or {} if nothing found.
    """
    try:
        ...
        print("extract_latest_json error:", e)
        return {}
...
def medical_chatbot_pipeline(query, chat_history, retrieved_chunks, context_followup, main_llm, judge_llm):
    """
    Liberal judge:
      - If query & context share the same core medical topic (strong OR weak), answer.
      - Only when 'none' → apologize and propose an ALT question anchored to the retrieved chunk.
    Anchored ALT:
      - ALT is a concise, standalone medical question that reuses exact noun-phrases from the chunk.
    """
    ...
    judge_prompt = f"""
            You are a liberal medical relevance judge.

            Return ONLY JSON:
            {{"topic_match":"strong|weak|none","sufficient":true/false,"why":"...","alternative":"<anchored medical question or ''>"}}

            Guidance:
            - "strong": same core condition/topic (e.g., preeclampsia vs. severe headache in pregnancy).
            - "weak": nearby angle or population (e.g., pregnancy complications in older age vs. pregnancy risk at 78).
            - "none": off-topic (e.g., programming, finance) or the context is mostly noise/symbols.

            "sufficient" is true for strong or weak. False only for "none".

            If insufficient:
            - "alternative" MUST be a short, standalone medical question derived from the TOP of <context>.
            - Use exact noun/condition phrases from the context to maximize retrieval re-hit.
            - 6–16 words, no placeholders, no pronouns, end with a question mark.
    ...
    decision = extract_latest_json(decision_raw) or {"topic_match":"none","sufficient":False,"alternative":""}
    print("decision_raw : ", decision_raw)
    print("decision : ", decision)
    topic_match = str(decision.get("topic_match","none")).lower()
    sufficient = bool(decision.get("sufficient", False))
    alt = (decision.get("alternative") or "").strip()

    # --- Proceed to answer if liberal match (strong or weak)
    if sufficient or topic_match in ("strong", "weak"):
        answer_prompt = f"""
            ...
            """
        answer = main_llm.invoke([HumanMessage(content=answer_prompt)]).content

        # capture newest follow-up for future "yes"
        m = re.search(r"Would you like to know about (.+?)\?", answer, flags=re.I)
        st.session_state.last_suggested = m.group(1).strip() if m else ""

        return answer

    # --- Insufficient: apologize + anchored alternative
    if alt:
        st.session_state.last_suggested = alt  # remember ALT for "yes"
