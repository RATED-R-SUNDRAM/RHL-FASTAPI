...
3. Categorize and act:
   - If only medical terminology, rephrase to "give some information about <term> if cure is available then cure."
...
answer_prompt_template = PromptTemplate.from_template(
    """
You are a highly skilled medical assistant. Provide answers solely based on the provided context.
If the context lacks sufficient information, respond with: "I apologize, but I do not have sufficient information to answer this question accurately."

Begin each answer with: **"According to <source>"** (where source is from metadata , extract the file name with its extension, 
e.g., if source is abc.pdf.pdf  retrun According to abc
      if source is def.pdf return According to def  ).
...
- Address unanswered aspects from the previous answer using only the context.
- Rely exclusively on the retrieved context, avoiding open web or general knowledge.
- Ensure coherence with recent chat history.
- Limit responses to 150 words unless explicitly instructed otherwise.
- Present answers in bullet points, not paragraphs.
- After answering return a follow-up question in the format "would you like to know about...<question from context not used in answer>".
...
def route_intent(user_message: str):
    ...
    print("phrased query is ", rewritten)
