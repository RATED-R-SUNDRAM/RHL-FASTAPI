...
- CHITCHAT: greetings, thanks, smalltalk, profanity, non-medical topics, or explicit "stop" requests

REFORMULATION RULES:
- If CHITCHAT: append "_chitchat" to rewritten_query, no corrections needed
...
chat_history: "Assistant: Would you like to know about jaundice?"
question: "no"
-> {{"classification":"CHITCHAT","reason":"user declining assistant suggestion","rewritten_query":"no_chitchat","corrections":{{}}}}
...
chat_history: ""
question: "hi, how are you?"
-> {{"classification":"CHITCHAT","reason":"greeting","rewritten_query":"hi, how are you?_chitchat","corrections":{{}}}}
...
chat_history: ""
question: "show me bitcoin price"
-> {{"classification":"CHITCHAT","reason":"non-medical topic","rewritten_query":"show me bitcoin price_chitchat","corrections":{{}}}}
...
def synthesize_answer(query: str, top_candidates: List[Dict[str, Any]], context_followup: str, main_llm: ChatOpenAI | None = None) -> str:
    ...
    print("[answer] sending synthesis prompt to LLM")
...
def classify_and_reformulate(chat_history: str, user_message: str) -> Tuple[str, str, str, Dict[str, str]]:
    """
    Combined classification and reformulation in a single LLM call.
    ...
    print("[combined] sending classification + reformulation prompt")
    
    try:
        # Use the same model as before (classifier_llm or fallback to llm)
        use_llm = classifier_llm if classifier_llm is not None else llm
        resp = use_llm.invoke([HumanMessage(content=prompt)]).content
        
        # Print raw response for debugging
        print("[COMBINED_RAW]:", resp)
        
        parsed = safe_json_parse(resp)
        print("[COMBINED_PARSED]:", parsed)
        ...
        if parsed:
            ...
            print("[combined] Failed to parse JSON response")
...
def handle_chitchat(user_message: str, chat_history: str) -> str:
    ...
    print("[chitchat] sending to chitchat model")
    try:
        return chitchat_llm.invoke([HumanMessage(content=prompt)]).content
    except Exception as e:
        print(f"[chitchat] LLM failed: {e}")
...
async def medical_pipeline_api(user_id: str, user_message: str, background_tasks: BackgroundTasks) -> Dict[str, Any]:
    ...
    # Combined classification + reformulation in single LLM call
    print("[pipeline] Step 1+2: classify_and_reformulate")
    label, reason, rewritten, correction = classify_and_reformulate(chat_history_context_for_llm, user_message)
    t3 = time.perf_counter()
    print(f"classify_and_reformulate took {t3 - t2:.2f} secs")
    print(f"[pipeline] classifier -> {label} ({reason})")
    print(f"[pipeline] rewritten: {rewritten} | correction: {correction}")
    ...
    try:
        ...
        print("[pipeline] Error during chitchat short-circuit check; continuing pipeline")
