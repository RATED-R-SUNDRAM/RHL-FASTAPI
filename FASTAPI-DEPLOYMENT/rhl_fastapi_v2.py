...
from pydantic import BaseModel # New import for request body validation
...
async def init_db():
    async with aiosqlite.connect("chat_history.db") as db:
        ...
        if 'intent' not in columns:
            await db.execute("ALTER TABLE chat_history ADD COLUMN intent TEXT NOT NULL DEFAULT 'unknown'")
...
async def get_history(user_id: str) -> Tuple[List[Tuple[str, str, str]], str]:
    async with aiosqlite.connect("chat_history.db") as db:
        # Fetch history pairs (question, answer, intent) and the most recent summary
        ...
        last_summary = ""
        for row in rows:
            history_pairs.append((row[0], row[1], row[2]))
            # The summary might be updated with each interaction, so we take the last one
            if row[3]: # only update if summary is not empty
                last_summary = row[3] 
...
# This function is adapted from Streamlit's session state logic
def update_chat_history_in_memory(current_history_pairs: List[Tuple[str, str, str]], current_summary: str, user_message: str, bot_reply: str, intent: str) -> Tuple[List[Tuple[str, str, str]], str]:
    ...
    if len(current_history_pairs) > MAX_VERBATIM_PAIRS:
        ...
        try:
            ...
            logging.error(f"Summarizer LLM failed: {e}")
            summary = ""
...
reformulate_prompt = PromptTemplate(
    input_variables=["chat_history", "last_suggested", "question", "classify"],
   template="""
...
def hybrid_retrieve(query: str, top_k_vec: int = 10, u_cap: int = 10) -> List[Dict[str, Any]]:
    ...
    """
    global embedding_model, pinecone_index, reranker # Access global models
    # 1) Vector search
...
# -------------------- JUDGE + ANSWER --------------------
def judge_sufficiency(query: str, candidates: List[Dict[str, Any]], judge_llm: ChatOpenAI = None, threshold_weak: float = 0.25) -> Dict[str, List[Dict[str, Any]]]:
    """
    ...
    for c in candidates:  # inspect up to 12. Iterate through all candidates initially
        ...
        try:
            ...
            logging.error(f"Error parsing judge LLM response or during fallback: {e}. Raw response: {resp}")
...
def synthesize_answer(query: str, top_candidates: List[Dict[str, Any]], context_followup: str, main_llm: ChatOpenAI = None) -> str:
    ...
    context = "\n\n".join(ctx_parts)
    # context_followup is already a string here from medical_pipeline_api
...
def reformulate_query(chat_history: str, user_message: str, classify_label: str, last_suggested: str = "") -> Tuple[str, str]:
    global llm # Access global llm
    logging.info("here")
    prompt = reformulate_prompt.format(chat_history=chat_history, last_suggested=last_suggested, question=user_message, classify=classify_label)
    logging.info("[reformulate] sending reformulation prompt")
    try:
        resp = llm.invoke([HumanMessage(content=prompt)]).content
        logging.info(f"resp is {resp}")
        parsed = safe_json_parse(resp)
        if parsed:
            return parsed.get("Rewritten", user_message), parsed.get("Correction", "")
    except Exception as e:
        logging.error(f"[reformulate] LLM failed: {e}")
    return user_message, ""

def handle_chitchat(user_message: str, chat_history: str) -> str:
    global chitchat_llm # Access global chitchat_llm
...
async def medical_pipeline_api(user_id: str, user_message: str, last_suggested: str = "") -> Dict[str, Any]:
    ...
    logging.info(f"[pipeline] chat_history (summary+last3): {chat_history_context_for_llm[:400]}")
    ...
    if judge["answer_chunks"]:
        ...
        updated_history_pairs, updated_summary = update_chat_history_in_memory(history_pairs, current_summary, user_message, answer, "answer")
        ...
        # Update and save history
        updated_history_pairs, updated_summary = update_chat_history_in_memory(history_pairs, current_summary, user_message, msg, "no_context")
...
async def chat_endpoint(chat_message: ChatMessage):
    try:
        response = await medical_pipeline_api(chat_message.user_id, chat_message.message, chat_message.last_suggested)
        return response
    except Exception as e:
        logging.error(f"Error in chat endpoint for user {chat_message.user_id}: {e}")
